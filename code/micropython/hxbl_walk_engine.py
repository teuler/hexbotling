...
from hxbl_tripod_gait import TripodGait
...
class WalkEngine(object):
  ...
  def spin(self):
    ...
    if st in [glb.STA_WALKING, glb.STA_REVERSING, glb.STA_TURNING]:
      ...
      #
      dt, ang = self._Gait.get_next_servo_pos()
      dt = min(max(int(dt /self._vel), 100), 3000)
      sm.move(cfg.SRV_ID, ang, dt)

    elif st == glb.STA_STOPPING:
      # Execute stop
      dt, ang = self._Gait.get_next_servo_pos(stop=True)
      sm.move(cfg.SRV_ID, ang, dt)
      self._state = glb.STA_IDLE
  ...
  def set_params(self, dir, rev, vel):
    ...
    """
    self._dir = max(min(dir, 1.0), -1.0) if dir else self._dir
    self._rev = bool(rev) if rev else self._rev
    self._vel = max(vel, 0.1) if vel else self._vel
  ...
  def get_servo_load_A(self, update=True):
    """ Returns the last current load of the servos; if `update` == True, it
        re-reads the value first. If `self._isUpdateStatusLEDs` == True, the
        assigned LED is updated as well.
    """
    if update:
