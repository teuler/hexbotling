...
# 2022-01-22, Joystick reporting improved (report also the first 0,0)
#
# ----------------------------------------------------------------------------
import pygame
import numpy as np
...
# ---------------------------------------------------------------------
class Stick(object):
  def __init__(self, jstick, xyIDs=[0,1], name="n/a"):
    self._ix = xyIDs[0]
    self._iy = xyIDs[1]
    self._JS = jstick
    self._ID = name
  ...
  def xy(self):
    ...
    dxy = abs(self._xy -temp)
    if dxy[0] < LIM and dxy[1] < LIM:
      return None
    self._xy = temp
    return self._xy
...
class Joystick(object):
  def __init__(self, joystick_ID):
    ...
    if joystick_ID >= 0 and joystick_ID < n:
      ...
      self.StickL = Stick(self._JS, [AXS_LX, AXS_LY], "L")
      self.StickR = Stick(self._JS, [AXS_RX, AXS_RY], "R")
